  ...
  return (
    <section className="mb-14 sm:mb-16" aria-label={`${languageName} problems`}>
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        {remainingCount > 0 ? (
          <p className="text-sm font-bold text-[var(--color-text-warm-muted)]">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {visibleProblems.map((problem, index) => (
          ...
          >
            ...
            <span className="mt-6 flex items-center justify-between gap-4 border-t border-[var(--color-border-subtle)] pt-4 text-sm font-bold text-[var(--color-text-muted)]">
              <span>{problem.solution}</span>
              <span className="text-[var(--color-primary-500)] transition group-hover:translate-x-1">
                Open
              </span>
            </span>
          </Link>
      ...
      {remainingCount > 0 ? (
        <div className="mt-8 flex justify-center pb-2 sm:mt-10 sm:pb-4">
          <button
            className="rounded-full border border-[var(--color-primary-500)] bg-[var(--color-primary-500)] px-6 py-3 text-sm font-black text-white shadow-[var(--shadow-card-hover)] transition hover:-translate-y-px hover:bg-[var(--color-primary-600)] focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[var(--color-primary-100)]"
            onClick={() =>
